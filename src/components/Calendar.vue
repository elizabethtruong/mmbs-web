        ...
        <v-card>
          <v-container>
            <v-form @submit.prevent="addEvent">
              ...
              <v-text-field v-model="end" type="time" label="end (required)"></v-text-field>
              <v-text-field v-model="venue" type="text" label="venue (required)"></v-text-field>
              <v-text-field v-model="price" type="text" label="price (require)"></v-text-field>
              <v-text-field v-model="details" type="text" label="details"></v-text-field>
              <v-btn type="submit" color="primary" class="mr-4" @click.stop="dialog = false">
                create event
              </v-btn>
...
export default {
  ...
  methods: {
    ...
    async addEvent () {
      if (this.eventName && this.clientNname && this.date && this.start && this.end && this.venue && this.price) {
        await db.collection("calEvent").add({
          eventName: this.eventName,
          clientName: this.clientName,
          date: this.date,
        ...
        alert('You must enter event name, client name, date, start time, end time, and venue')
      }
    },
    editEvent (ev) {
      this.currentlyEditing = ev.id
